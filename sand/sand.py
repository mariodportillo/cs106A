...
def do_move(grid, x_from, y_from, x_to, y_to):
    ...
    # sets new square to be (x_to, y_to)
...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    False
    """
    # If it is out of bound or there is something where it is trying to go it will return False
    if not grid.in_bounds(x_to, y_to) or grid.get(x_to, y_to) != None:
        return False

    # Checks if we are going diagonal to the right
    if y_from + 1 == y_to and x_from + 1 == x_to:

        # Checks if there is a rock to the right of original starting spot
        if grid.get(x_from + 1, y_from) != None:
            return False

    # Checks if we are going diagonal to the left
    if y_from + 1 == y_to and x_from - 1 == x_to:

        # Checks if there is a rock to the left of original starting spot
        if grid.get(x_from - 1, y_from) != None:
            return False
...
def do_gravity(grid, x, y):
    ...
    # checks ot make sure (x,y) is not none
    if grid.get(x, y) == None:
        return grid

    # checks if down is clear then falls down
    if is_move_ok(grid, x, y, x, y + 1):
        do_move(grid, x, y, x, y + 1)
        return grid

    # checks if left down is clear and goes down
    if is_move_ok(grid, x, y, x - 1, y + 1):
        do_move(grid, x, y, x - 1, y + 1)
        return grid

    # checks if right down is clear and goes down
    if is_move_ok(grid, x, y, x + 1, y + 1):
        do_move(grid, x, y, x + 1, y + 1)
        return grid


    # if nothing is clear then it will return grid
    return grid
...
def do_brownian(grid, x, y, brownian):
    ...
    # sets our num variable to random digit in between 0-99
    num = random.randrange(100)

    # if the (x, y) is equal to an "s" and num is less than brownian
    if grid.get(x, y) == "s" and num < brownian:

        #  coin variable is set to random number between 0 and 1
        coin = random.randrange(2)

        # if coin is equal to 0 and move is okay to move left then it will move to the left
        if coin == 0 and is_move_ok(grid, x, y, x-1, y):
            do_move(grid, x, y, x-1, y)
            return grid

        # if coin is equal to 1 and move is okay to move right then it will move to the right
        if coin == 1 and is_move_ok(grid, x, y, x + 1, y):
            do_move(grid, x, y, x + 1, y)
            return grid

        # if neither is true then it will just simply return the original grid
        return grid
...
def do_whole_grid(grid, brownian):
    ...
    """
    # goes through reverse range of grid.height which is 1 -> only goes down once then goes through all x -> grid.width x
    for y in reversed(range(grid.height)):
        for x in range(grid.width):
            # checks if grid is "s"
            if grid.get(x, y) == 's':
                # applies gravity to that grid and that (x,y)
                do_gravity(grid, x, y)

                # applies the brownian affect to that specific (x,y)
